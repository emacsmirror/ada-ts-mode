...
#+HTML: <a href="https://stable.melpa.org/#/ada-ts-mode"><img alt="MELPA Stable" src="https://stable.melpa.org/packages/ada-ts-mode-badge.svg"/></a>
...
and =textDocument/formatting= requests.  This corresponds to the main
indentation amount (i.e., overriding the =--indentation= switch for
=GNATpp= or the =Indentation= package setting for =GNATformat=).
...
While ~ada-ts-mode~ is a fully capable major mode, users may desire to
utilize an LSP server (i.e., Ada Language Server) in order to enhance
their experience.  This can provide enhanced capabilities for
completion, cross-referencing, documentation, etc.  It's up to the
user to configure the LSP client for an ~ada-ts-mode~ buffer, typically
through the use of the ~ada-ts-mode-hook~ to enable the LSP client minor
mode.  Those clients which utilize directory local variables for
configuration (e.g., GPR project filename), may need to hook into the
...
The Ada Language Server allows for client-agnostic configuration
through a project-specific configuration file (=.als.json=) in addition
to a user-specific configuration file (=config.json=).  This approach is
recommended such that users aren't reliant on an LSP client's specific
configuration peculiarities.  Using this approach also allows the user
to easily switch between LSP clients (and even editors) with a unified
configuration.  Refer to the Ada Language Server's documentation for
specific details on the format and location of these files.

Some Ada Language Server features are known to be problematic with
Emacs (such as on-type formatting).  Additionally, some Emacs LSP
clients have default settings which aggressively override similar
functionality provided by ~ada-ts-mode~.  As a result, ~ada-ts-mode~
provides a way for the settings of each LSP client to be customized
specifically for use within an ~ada-ts-mode~ buffer.  The customizations
are specific to the ~ada-ts-mode~ buffer and will not impact the
settings for other major modes using the same LSP client.  This allows
the LSP client's global settings to be retained for other major modes
while providing customization for ~ada-ts-mode~.

A default set of configurations have been provided out-of-the-box to
optimize the experience with each supported LSP client.  The following
user options may be adjusted as desired.  For each LSP client, if
their user option(s) are set to ~nil~, no customization for ~ada-ts-mode~
will be applied, allowing the LSP client's global settings to be used
instead.

- User Option: ada-ts-lspclient-eglot-stay-out-of ::
  An ~ada-ts-mode~ version of ~eglot~'s ~eglot-stay-out-of~ setting.  The
  global value of ~eglot-stay-out-of~ is merged with this user option to
  determine the buffer-local value.

- User Option: ada-ts-lspclient-eglot-ignored-server-capabilities ::
  An ~ada-ts-mode~ version of ~eglot~'s ~eglot-ignored-server-capabilities~
  setting.  The global value of ~eglot-ignored-server-capabilities~ is
  merged with this user option to determine the buffer-local value.

- User Option: ada-ts-lspclient-lsp-mode-settings-alist ::
  An attribute list of ~lsp-mode~ settings whose key should be an
  ~lsp-mode~ variable to set and the value is the corresponding value
  which the variable should receive.  These variables are set
  buffer-locally to avoid impacting settings outside of ~ada-ts-mode~
  buffers.

A set of customizations have also been provided for ~eglot~ semantic
highlighting.  For ~lsp-mode~, similar customizations are part of the
~lsp-mode~ package itself.

- User Option: ada-ts-lspclient-eglot-semantic-token-types ::
  An ~ada-ts-mode~ version of ~eglot~'s ~eglot-semantic-token-types~
  setting.  The global value of ~eglot-semantic-token-types~ is
  overridden with this user option to determine the buffer-local
  value.  Only token types provided in this list are used for semantic
  highlighting in ~ada-ts-mode~ buffers.  Other token types not
  specified are ignored.

- User Option: ada-ts-lspclient-eglot-semantic-token-modifiers ::
  An ~ada-ts-mode~ version of ~eglot~'s ~eglot-semantic-token-modifiers~
  setting.  The global value of ~eglot-semantic-token-modifiers~ is
  overridden with this user option to determine the buffer-local
  value.  Only token modifiers provided in this list are used for
  semantic highlighting in ~ada-ts-mode~ buffers.  Other token modifiers
  not specified are ignored.

- User Option: ada-ts-lspclient-eglot-semantic-token-face-overrides ::
  An attribute list of ~eglot~ semantic token type and semantic token
  modifier face overrides.  The ~eglot~ provided face settings for
  semantic token types and semantic token modifiers can be overridden
  with different faces or attributes of faces in ~ada-ts-mode~ buffers.
  Semantic token types and semantic token modifiers not listed will
  retain their default faces as specified in the ~eglot~ package.

** Adding Support for an Additional LSP Client

In order to integrate functionality provided by the Ada Language
Server (such as completion), ~ada-ts-mode~ must interact with an active
...
There are already two LSP clients supported by ~ada-ts-mode~, ~eglot~ and
~lsp-mode~, so it is unlikely that a user will need to provide their own
support for a new client.  In the event that is not the case, a
generic interface is used to support different LSP clients.  This
interface is described in =ada-ts-lspclient.el=.  There are separate
files for each supported LSP client which implement those interfaces.
These separate files are configured to automatically load when both
...
of the interfaces described in =ada-ts-lspclient.el=.  In addition, a
parameter-less function for that client should be added to the
~ada-ts-lspclient-find-functions~ hook variable (via ~add-hook~).  That
function, when invoked, should determine if the LSP client is active
in the current buffer, and when active, return an LSP client instance
(which can be used to dispatch on the client-specific methods), else
return ~nil~.

* Ada Language Server

Functionality provided by the Ada Language Server is seamlessly
integrated into the corresponding functional areas as appropriate
(i.e., indentation, cross-references, etc.).  However, there is other
support ~ada-ts-mode~ provides which is specific to managing the Ada
Language Server itself, not the functionality it provides.

- Command: ada-ts-als-find-user-config-file ::
  Find Ada Language Server user configuration file.  If the variable
  =$XDG_CONFIG_HOME= exists, the user configuration file is
  =$XDG_CONFIG_HOME/als/config.json=.  Otherwise, the user configuration
  file is =~/.config/als/config.json=.  If the file does not exist, a
  new file will be visited.

- Command: ada-ts-als-find-workspace-config-file ::
  Find Ada Language Server workspace configuration file.  The file is
  found in the LSP client defined workspace root directory, when an
  LSP client is active, otherwise the file is found relative to the
  project root as determined by =project.el=.  If the file does not
  exist, a new file will be visited.

- Command: ada-ts-als-show-composite-config ::
  Show the composite JSON-formatted configuration for the Ada Language
  Server combining the user configuration file (when exists), the
  workspace configuration file (when exists) and the LSP client
  specific configuration (when an LSP client is active).  This
  provides a holistic view of the separate parts that comprise the
  configuration, which can be helpful when troubleshooting
  configuration problems.  Overlapping configuration from these
  sources are combined based on priority, such that the LSP client
  configuration is higher priority than the workspace configuration,
  which is higher priority than the user configuration.  This is
  consistent with how the Language Server resolves overlapping
  configuration.  Note that the Ada Language Server has a default
  configuration, when not specified by any of these sources.  Refer to
  the Ada Language Server documentation for full details.
...
Prior to Org Mode 9.8, when Org Mode doesn't know the major mode for
the language of a source block, it will guess by appending =-mode= to
the end of the language name.  If a language name of =ada= is used in
the source block, it will look for a major mode named ~ada-mode~.  This
default behavior doesn't work when using Tree-Sitter enabled modes.

As of Org Mode 9.8, Tree-Sitter enabled modes are properly detected
and mapped correctly from the corresponding language.  However, if you
are still using an older version, you can explicitly configure Org
Mode to map to the Tree-Sitter major mode using the customization
variable =org-src-lang-modes=.

The following can be added to your configuration to detect the version
of Org Mode being used, and conditionally apply the workaround:

#+BEGIN_SRC emacs-lisp
  (with-eval-after-load 'org-src
    (when (version< (org-version) "9.8")
